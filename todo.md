...
* [x] **M8.1 抓取版本号/发布日期/Release Notes URL**
  **Deps:** M3.1.3
  背景：模块页默认“最新大版本 x.y”需要权威发布日期序列。

  * [x] **M8.1.1 定义输出结构**（`data/meta/versions.json`）
    **Deps:** M4.2.1
  * [x] **M8.1.2 实现抓取器与解析器**（容错 DOM 变化）
    **Deps:** M8.1.1
  * [x] **M8.1.3 计算“最新大版本 x.y”**（按发布日期）
    **Deps:** M8.1.2

* [x] **M8.2 版本元数据接入前端**
  **Deps:** M8.1, M2.2

  * [x] **M8.2.1 构建期将 versions.json 打包为可 import 模块**
    **Deps:** M8.1.2
  * [x] **M8.2.2 模块页默认版本范围使用“最新大版本 x.y”**
    **Deps:** M8.2.1, M2.4.2
...
* [x] **M9.1 接入 Google CSE（配额与分页）**
  **Deps:** M3.1.3

  * [x] **M9.1.1 定义请求预算与分页参数**（150 keywords/day；2 pages/keyword）
    **Deps:** M3.1.3
  * [x] **M9.1.2 实现查询执行器**（记录每次调用与剩余预算）
    **Deps:** M9.1.1

* [x] **M9.2 关键词生成（模块权重优先筛选）**
  **Deps:** M6.3, M3.1.3

  * [x] **M9.2.1 从 commit/PR/ReleaseNotes 抽取候选关键词**（术语/模块/标签/版本）
    **Deps:** M6.3.4
  * [x] **M9.2.2 候选>150 时按模块权重筛选**（高权重模块优先）
    **Deps:** M3.1.3, M9.2.1

* [x] **M9.3 抓取页面并生成证据摘录（≤300 词）**
  **Deps:** M9.1, M4.2

  * [x] **M9.3.1 下载 HTML（仅用于抽取摘录，不做全量保存）**
    **Deps:** M9.1.2
  * [x] **M9.3.2 段落切分与相关性评分（TF-IDF + 余弦）**
    **Deps:** M4.2.1
  * [x] **M9.3.3 取 Top3 段拼接并截断到 300 词**
    **Deps:** M9.3.2
  * [x] **M9.3.4 写入 sources（候选来源，后续按可信度选前 3）**
    **Deps:** M4.4.2

* [x] **M9.4 近重复合并（0.92）**
  **Deps:** M9.3

  * [x] **M9.4.1 计算摘录向量（TF-IDF）**
    **Deps:** M9.3.3
  * [x] **M9.4.2 相似度>0.92 合并并保留转载来源列表**
    **Deps:** M9.4.1
...
* [x] **M10.1 提取 og:image/twitter:image**
  **Deps:** M9.3.1

  * [x] **M10.1.1 从 HTML meta 中提取 URL**
    **Deps:** M9.3.1
  * [x] **M10.1.2 失败时返回空（由前端占位）**
    **Deps:** M10.1.1

* [x] **M10.2 下载与转码压缩（≤512px、≤200KB、JPEG）**
  **Deps:** M10.1

  * [x] **M10.2.1 下载图片资源**（处理重定向与超时）
    **Deps:** M10.1.1
  * [x] **M10.2.2 转码为 JPEG 并缩放**（最长边 512）
    **Deps:** M10.2.1
  * [x] **M10.2.3 质量/大小控制到 ≤200KB**（迭代降质或二次缩放）
    **Deps:** M10.2.2
  * [x] **M10.2.4 每条最多 3 张缩略图**（超出截断）
    **Deps:** M10.2.3
  * [x] **M10.2.5 落盘路径规范**：`assets/thumbs/<item_id>_<n>.jpg`
    **Deps:** M4.3.2

* [x] **M10.3 前端列表展示（卡片左侧小图）**
  **Deps:** M2.4.2, M10.2

  * [x] **M10.3.1 卡片组件增加缩略图区域**
    **Deps:** M2.4.2
  * [x] **M10.3.2 缺失缩略图时显示占位图**
    **Deps:** M10.3.1
...
* [x] **M11.1 LLM 客户端封装（OpenAI 优先，DeepSeek 备用）**
  **Deps:** M3.1.3

  * [x] **M11.1.1 统一接口：generate(prompt)->result**（含超时、重试、fallback）
    **Deps:** M3.1.3
  * [x] **M11.1.2 记录模型与时间戳**（写入 `ai` 字段）
    **Deps:** M11.1.1

* [x] **M11.2 多标签分类（纯多标签 + 置信度）**
  **Deps:** M6.3, M9.3, M11.1

  * [x] **M11.2.1 输入拼装**（title/摘要/证据摘录/文件路径/module_code）
    **Deps:** M6.3.2, M9.3.3
  * [x] **M11.2.2 输出结构定义**（module_system[]/tags[]/confidence）
    **Deps:** M4.1.2
  * [x] **M11.2.3 低置信度标注规则**（<0.5 仅详情标注）
    **Deps:** M11.2.2

* [x] **M11.3 benefits 生成（中英）**
  **Deps:** M11.1, M4.2

  * [x] **M11.3.1 一句话 + 3 要点**（性能/质量/工作流）
    **Deps:** M4.1.2
  * [x] **M11.3.2 写入 front matter 与正文 AI 区**（标注 AI 生成）
    **Deps:** M4.2.2, M11.1.2

* [x] **M11.4 risks 生成（中英）**
  **Deps:** M11.1, M4.2

  * [x] **M11.4.1 注意事项清单 ≤5 条**
    **Deps:** M4.1.2
  * [x] **M11.4.2 写回并标注 AI 生成**
    **Deps:** M4.2.2, M11.1.2

* [x] **M11.5 证据摘录翻译（中文）**
  **Deps:** M9.3, M4.2, M11.1

  * [x] **M11.5.1 翻译策略实现**：保留术语/缩写；类名/路径/代码不翻译；缩写不补全
    **Deps:** M4.2.2
  * [x] **M11.5.2 写入 Evidence 区（上中文下英文）并标注 AI 翻译**
    **Deps:** M11.5.1

* [x] **M11.6 写回一致性与校验**
  **Deps:** M11.2, M11.3, M11.4, M11.5, M4.4

  * [x] **M11.6.1 更新 `ai` 字段（字段级标记/模型/时间）**
    **Deps:** M11.1.2
  * [x] **M11.6.2 跑 validate-items 确保 schema 合规**
    **Deps:** M4.4.1
...
* [x] **M12.1 P1 规则定义与配置化**
  ...
  * [x] **M12.1.1 定义可解释指标**（改动类型/影响范围/来源数量/冲突标志等）
    **Deps:** M4.1.2
  * [x] **M12.1.2 写入 `config/pipeline.yaml` 或独立 `config/scoring.yaml`**
    **Deps:** M12.1.1

* [x] **M12.2 P1 计算器实现**
  **Deps:** M12.1, M6.3, M7.4, M8.1

  * [x] **M12.2.1 从 item 聚合所需特征**（change_type、sources、高可信冲突等）
    **Deps:** M6.3.5, M7.4.1
  * [x] **M12.2.2 输出 score 与解释字段**（用于前端 tooltip 或日志）
    **Deps:** M12.2.1
  * [x] **M12.2.3 写入索引字段（不强制写回 Markdown）**
    **Deps:** M12.2.2
...
* [x] **M13.1 轻量基础索引生成器（≤50MB）**
  **Deps:** M12.2, M4.2

  * [x] **M13.1.1 定义基础索引字段集**（标题/摘要/标签/模块/版本/改动类型/发布日期 + 少量摘录）
    **Deps:** M4.1.2
  * [x] **M13.1.2 生成 `index_base.ts/js` 可 import 模块**（避免 file:// fetch）
    **Deps:** M13.1.1
  * [x] **M13.1.3 体积监控与失败阈值**（超 50MB 写日志并继续发布）
    **Deps:** M13.1.2

* [x] **M13.2 中文分词（jieba）**
  **Deps:** M13.1

  * [x] **M13.2.1 对中文字段分词**（含翻译文本）
    **Deps:** M13.1.1
  * [x] **M13.2.2 输出 tokens 给 FlexSearch**
    **Deps:** M13.2.1

* [x] **M13.3 FlexSearch 索引构建与导出**
  **Deps:** M13.2

  * [x] **M13.3.1 定义字段权重**（标题>摘要>标签）
    **Deps:** M13.1.1
  * [x] **M13.3.2 构建索引并导出可 import 的 JS 模块**
    **Deps:** M13.3.1

* [x] **M13.4 按模块分块导出详情数据（dynamic import）**
  **Deps:** M4.2, M8.1

  * [x] **M13.4.1 为每个 module_system 生成 `chunk_<ms>.ts/js`**
    **Deps:** M3.1.1
  * [x] **M13.4.2 chunk 内容包含详情页所需完整字段**（Evidence/GitHub/AI/paths/thumbs）
    **Deps:** M4.2.2, M10.2.5
...
* [x] **M14.1 模块目录页：真实模块统计**
  **Deps:** M13.1, M3.2

  * [x] **M14.1.1 从基础索引聚合模块数量/最近更新时间**
    **Deps:** M13.1.2
  * [x] **M14.1.2 模块卡片渲染（可点击进入模块页）**
    **Deps:** M2.2.3, M14.1.1

* [x] **M14.2 模块页：默认最新大版本 + 20 条 + 无限滚动**
  **Deps:** M8.2, M13.4, M2.4

  * [x] **M14.2.1 计算模块页默认版本过滤（x.y）**
    **Deps:** M8.1.3
  * [x] **M14.2.2 加载模块 chunk 并过滤到 x.y**
    **Deps:** M13.4.2
  * [x] **M14.2.3 按 P1 排序，首屏 20，滚动加载后续**
    **Deps:** M12.2.3, M2.4.3

* [x] **M14.3 模块页内搜索（标题/摘要/标签）**
  **Deps:** M14.2, M2.4.1

  * [x] **M14.3.1 实现本地过滤/评分（仅三字段）**
    **Deps:** M13.1.2
  * [x] **M14.3.2 搜索结果与无限滚动协同**（搜索时重置分页窗口）
    **Deps:** M14.3.1

* [x] **M14.4 详情页：Evidence/GitHub/AI/文件路径/缩略图**
  **Deps:** M13.4, M10.3, M11.6

  * [x] **M14.4.1 按 id 加载所属模块 chunk 并定位 item**
    **Deps:** M13.4.2
  * [x] **M14.4.2 Evidence 卡片渲染（中文在上、英文在下）**
    **Deps:** M2.3.3, M11.5.2
  * [x] **M14.4.3 GitHub 区渲染（PR/Issue 摘要+评论）**
    **Deps:** M7.4.1
  * [x] **M14.4.4 文件路径列表（前 30 + “还有 X”）**
    **Deps:** M6.3.2
  * [x] **M14.4.5 缩略图展示与占位**
    **Deps:** M10.3.2
...
* [x] **M15.1 搜索页接入 FlexSearch**
  **Deps:** M13.3

  * [x] **M15.1.1 搜索输入与 debounce**
    **Deps:** M2.2.3
  * [x] **M15.1.2 调用索引查询并返回候选集合**
    **Deps:** M13.3.2

* [x] **M15.2 结果卡片字段与高亮片段**
  **Deps:** M13.1

  * [x] **M15.2.1 卡片字段：标题摘要、版本&日期、模块、改动类型、可信度、命中片段**
    **Deps:** M13.1.1
  * [x] **M15.2.2 命中高亮（从 ≤300 摘录截取）**
    **Deps:** M15.1.2

* [x] **M15.3 排序：重要性 P1 优先**
  **Deps:** M12.2

  * [x] **M15.3.1 将 P1 score 作为首排序键**
    **Deps:** M12.2.3
  * [x] **M15.3.2 同分再按匹配度（可选）**
    **Deps:** M15.1.2
...
* [x] **M16.1 收藏数据结构与 API**
  ...
  * [x] **M16.1.1 localStorage 存 `{id, ts}`**
    **Deps:** M2.3.2
  * [x] **M16.1.2 收藏/取消收藏操作封装**
    **Deps:** M16.1.1

* [x] **M16.2 详情页收藏按钮接入**
  **Deps:** M14.4

  * [x] **M16.2.1 详情页按钮与状态同步**
    **Deps:** M16.1.2
...
* [x] **M17.1 日报生成器（构建期）**
  ...
  * [x] **M17.1.1 定义“当天新增/更新条目”的判定**（按抓取时间或 commit 时间）
    **Deps:** M6.3.1
  * [x] **M17.1.2 按 module_system 分组并取 P1 前 10**
    **Deps:** M12.2.3
  * [x] **M17.1.3 输出 `reports/daily/YYYY-MM-DD.md`**
    **Deps:** M17.1.2

* [x] **M17.2 日报页展示**
  **Deps:** M2.2.3, M17.1

  * [x] **M17.2.1 读取日报产物并渲染模块分组**
    **Deps:** M17.1.3
...
* [x] **M18.1 统一日志事件结构与写入**
  ...
  * [x] **M18.1.1 定义 `reports/logs/index.json` 结构**（每次构建一条摘要+样例错误）
    **Deps:** M1.1.2
  * [x] **M18.1.2 每个 pipeline step 写入日志**（GitHub/Release/CSE/Thumb/AI/Index/Web）
    **Deps:** M18.1.1

* [x] **M18.2 首页仅显示最近一次构建状态**
  **Deps:** M2.2.3, M18.1

  * [x] **M18.2.1 首页状态条：成功/失败/截断/配额耗尽**
    **Deps:** M18.1.2
  * [x] **M18.2.2 “数据可能不完整”条件**（>200 commit 截断等）
    **Deps:** M6.2.3

* [x] **M18.3 日志页展示（摘要 + 展开样例）**
  **Deps:** M2.2.3, M18.1

  * [x] **M18.3.1 渲染构建列表**（时间、步骤、错误数）
    **Deps:** M18.1.2
  * [x] **M18.3.2 展开显示前 20 条错误样例**
    **Deps:** M18.3.1
...
* [x] **M19.1 Actions workflow 基础**
  **Deps:** M1.1.1

  * [x] **M19.1.1 定义 schedule（UTC 0:00）+ workflow_dispatch**
    **Deps:** M1.1.1
  * [x] **M19.1.2 定义 Secrets 清单**（PAT/OpenAI/DeepSeek/CSE Key/cx）
    **Deps:** M19.1.1

* [x] **M19.2 运行脚本编排（pipeline 主入口）**
  **Deps:** M6, M7, M8, M9, M10, M11, M12, M13, M17, M18

  * [x] **M19.2.1 编写 `tools/pipeline/run.ts|py` 主入口**（按阶段执行并捕获错误）
    **Deps:** M18.1.2
  * [x] **M19.2.2 实现“部分失败也发布”**（单阶段失败不中断全局；写日志）
    **Deps:** M19.2.1

* [x] **M19.3 构建站点与提交产物（GITHUB_TOKEN）**
  **Deps:** M2.1.3, M13, M14

  * [x] **M19.3.1 Actions 中构建前端产物**（Vite build）
    **Deps:** M2.1.3
  * [x] **M19.3.2 提交生成内容回仓库**（只用 `GITHUB_TOKEN` 写入）
    **Deps:** M19.3.1

* [x] **M19.4 验证离线可用性**
  **Deps:** M19.3

  * [x] **M19.4.1 本地 `git pull` 后双击 `index.html` 可用**（无 fetch/CORS 问题）
    **Deps:** M19.3.2
...
* [x] **M20.1 黄金路径验收：模块浏览**
  ...
  * [x] **M20.1.1 模块目录 → 模块页（最新大版本）→ 搜索 → 详情**（Evidence/文件路径/来源齐全）
    **Deps:** M14.4

* [x] **M20.2 搜索验收：全局搜索（重要性优先）**
  **Deps:** M15

  * [x] **M20.2.1 搜索关键术语 → 结果卡片字段完整 → 进入详情**
    **Deps:** M15.2

* [x] **M20.3 收藏验收**
  **Deps:** M16

  * [x] **M20.3.1 收藏后刷新页面仍存在；收藏页时间倒序**
    **Deps:** M16.3

* [x] **M20.4 日报验收**
  **Deps:** M17

  * [x] **M20.4.1 日报按模块分组、每模块 ≤10、按 P1 排序**
    **Deps:** M17.2.1

* [x] **M20.5 异常可追溯验收（部分失败也发布）**
  **Deps:** M18, M19

  * [x] **M20.5.1 模拟 CSE key 失效：站点仍发布；首页提示异常；日志页可见样例**
    **Deps:** M18.3.2, M19.2.2
  * [x] **M20.5.2 模拟 commit>200 截断：首页提示“数据可能不完整”**
    **Deps:** M6.2.3, M18.2.2
...
* [x] **M21.1 开发者文档**
  ...
  * [x] **M21.1.1 README：本地运行/构建/调试 pipeline**
    **Deps:** M19.4.1
  * [x] **M21.1.2 配置手册：module_mapping/tag_synonyms/scoring**
    **Deps:** M3.1, M12.1.2

* [x] **M21.2 运维文档（Actions）**
  **Deps:** M19

  * [x] **M21.2.1 Secrets 说明与最小权限原则**（PAT 只读 UE；写入用 GITHUB_TOKEN）
    **Deps:** M19.1.2, M19.3.2
  * [x] **M21.2.2 常见失败排查**（看 `reports/logs/index.json` 与首页提示）
    **Deps:** M18.3.2
