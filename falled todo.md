...
- [ ] **M1.1 建立仓库与目录结构**  
  **Deps:** 无  
  ...
  - [ ] **M1.1.3 添加基础文档与运行说明**（README + 架构草图）  
    **Deps:** M1.1.2  

- [ ] **M1.2 工具链与规范**  
  **Deps:** M1.1  
  背景：统一格式化/校验，保证多人协作与 Actions 自动化稳定。  
  - [ ] **M1.2.1 前端 lint/format**（ESLint + Prettier + EditorConfig）  
    **Deps:** M1.1.2  
  - [ ] **M1.2.2 Node/Python 代码质量工具**（可选：eslint/ruff）  
    **Deps:** M1.1.2  
  - [ ] **M1.2.3 提交规范**（可选：commitlint）与 CI 基础检查  
    **Deps:** M1.2.1  
...
- [ ] **M2.1 初始化前端工程（React + Vite + TS）**  
  **Deps:** M1.1.2  
  背景：PRD 要求离线双击打开，需 SPA + hash 路由。  
  - [ ] **M2.1.1 创建 Vite React TS 项目**（`apps/web`）  
    **Deps:** M1.1.2  
  - [ ] **M2.1.2 安装依赖**：MUI、Zustand、FlexSearch、路由（HashRouter）  
    **Deps:** M2.1.1  
  - [ ] **M2.1.3 配置基础构建输出目录**（如 `site/` 或 `dist/`）  
    **Deps:** M2.1.1  

- [ ] **M2.2 路由与页面骨架（无数据也能跑通）**  
  **Deps:** M2.1  
  背景：先把信息架构落地为可访问页面，后续逐步替换为真实数据。  
  - [ ] **M2.2.1 定义 hash 路由**：`#/`、`#/module/:ms`、`#/item/:id`、`#/search`、`#/favorites`、`#/daily`、`#/logs`  
    **Deps:** M2.1.2  
  - [ ] **M2.2.2 布局组件**：顶栏导航 + 内容区（响应式）  
    **Deps:** M2.2.1  
  - [ ] **M2.2.3 页面占位 UI**（模块目录/模块页/详情/搜索/收藏/日报/日志）  
    **Deps:** M2.2.2  

- [ ] **M2.3 语言显示开关（全站记忆）**  
  **Deps:** M2.2  
  背景：PRD 规定中英同屏（上中下英）且可切换，需全站状态持久化。  
  - [ ] **M2.3.1 Zustand store：languageMode（zh/en/both）**  
    **Deps:** M2.2.2  
  - [ ] **M2.3.2 localStorage 持久化与默认值（both）**  
    **Deps:** M2.3.1  
  - [ ] **M2.3.3 UI 控件：顶栏语言切换**（影响所有页面文本渲染）  
    **Deps:** M2.3.2  

- [ ] **M2.4 关键交互骨架（模块页）**  
  **Deps:** M2.2  
  背景：黄金路径是模块浏览，先把交互容器搭好（20 条 + 无限滚动 + 模块内搜索）。  
  - [ ] **M2.4.1 模块页：模块内搜索框组件**（仅作用于模块页）  
    **Deps:** M2.2.3  
  - [ ] **M2.4.2 模块页：列表卡片组件（移动端适配）**  
    **Deps:** M2.4.1  
  - [ ] **M2.4.3 无限滚动容器**（IntersectionObserver + “加载中”占位）  
    **Deps:** M2.4.2  
